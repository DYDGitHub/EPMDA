...
from LySpoon.DataStream.InteractionDataSource import InteractionDataSource
from sklearn.neural_network import MLPRegressor
import numpy as np

from LySpoon.LinkPrediction.PredictNewPairs import PredictNewPairs
...
class AsymModel(object):
    ...
    def __init__(self, array, directory, numFeature, numFold):
        ...
        self.graph = nx.from_numpy_matrix(self.symArray)
...
class PredictNewPairs(object):

    def __init__(self, trainArray, directory, numFeatures):
        """
        ...
        disease = np.loadtxt(directory + "Gaussian_disease.csv", dtype=np.float, delimiter=",")
        miRNA = np.loadtxt(directory + "Gaussian_miRNA.csv", dtype=np.float, delimiter=",")
        top = np.hstack((miRNA, trainArray))
        bottom = np.hstack((trainArray.T, disease))
        self.symArray = np.vstack((top, bottom))

        self.numFeature = numFeature
        self.directory = directory
        self.originValues = self.hamilton(np.linalg.eigvals(self.symArray).real)
        self.features = [[None for i in range(self.numCol)] for j in range(self.numRow)]

    def hamilton(self, eigenValues):
        """
        ...
        values = []
        start = 3
        for k in range(start, self.numFeature + start):
            wul = sum(eigenValues ** k)
            if wul > 0:
                values.append(math.log(wul))
            else:
                values.append(0.0)
    ...
    def readFeatures(self):
        ...
        @return:
        """
        eigenValues = np.load(self.directory + "/EigenValue.npy")
        for i in range(self.numRow):
            for j in range(self.numCol):
                values = self.hamilton(eigenValues[i][j])
                if self.trainArray[i][j] == 1:
                    self.features[i][j] = [self.originValues[k] - values[k] for k in range(self.numFeature)]

                elif self.trainArray[i][j] == 0:
                    self.features[i][j] = [values[k] - self.originValues[k] for k in range(self.numFeature)]
    ...
    def toNumArray(self):
        ...
        for rowIndex in range(self.numRow):
            for colIndex in range(self.numCol):
                features = self.features[rowIndex][colIndex]
...
def calAUC(trainArray, probeArray, fold, queue1, queue2, method):
    """
...
if __name__ == '__main__':
    ...
    directory = "data/"
    featuresPath = directory + 'featureForNew/'
    ...
    numRow, numCol = interactions.dimensions()
    numInteractions = interactions.numInteractions
    array = interactions.toArrays()
    start = time.time()

    predictNew = PredictNewPairs(array, featuresPath + str(1), numFeature)
    if len(os.listdir(featuresPath)) != 1:
        print "calculate features"
        calFeatures()

    print "read Feature"
    predictNew.readFeatures()

    X_train, X_test, y_train = predictNew.toNumArray()

    min_max_scaler = preprocessing.MinMaxScaler()
    X_train_minmax = min_max_scaler.fit_transform(X_train)
    X_test_minmax = min_max_scaler.transform(X_test)
    lr = MLPRegressor(hidden_layer_sizes=(numFeature * hiddenTimes,))
    lr.fit(X_train_minmax, y_train)
    ...
    print('lenght of values:' + str(len(values)))

    workBook = xlsxwriter.Workbook(directory + 'resultsForNew/newPredicted.xlsx')
    ...
    for i in range(numRow):
        for j in range(numCol):
            if array[i][j] == 0:
                workSheet.write(i, j, values[count])
                count += 1
            else:
                workSheet.write(i, j, -10000)
    print(count)
