...
class EPMDA(object):
    ...
    def __init__(self, array, directory, numFeature, numFold=1):
        """
    ...
    def calFeatures(self):
        ...
        fileName = self.directory + "/featuresForNew" + str(1) + ".npy"
        ...
        for rowIndex in range(self.numRow):
            for colIndex in range(self.numCol):
                ...
                if self.trainArray[rowIndex][colIndex] == 0:
                    ...
                    pairIndex.append([rowIndex, colIndex])

        return np.array(trainData), np.array(testData), np.array(trainLabel), pairIndex


def predictNew(trainArray, method, directory, numFeatures):
    """
    ...
    simpleModel = SimpleModel(trainArray, directory + "featuresForNew/", numFeature)
    simpleModel.readFeatures(1)

    X_train, X_test, y_train, pairIndexs = simpleModel.toNumArray()
    ...
    for i in range(len(values)):
        results.append([pairIndexs[0], pairIndexs[1], values[i]])

    np.savetxt(directory + "resultsForNew/newPredicted.txt", np.array(results))


def calFeatures(trainArray, directory, numFeatures):
    """
    ...
    :return:
    """
    asymModel = EPMDA(trainArray, directory, numFeatures)
