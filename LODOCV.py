...
"""
file: model for read Cluster data set

@version: $1.0$
@author: Ya-Dong Dong (yddong.gm@gmail.com)
@contact: Ya-Dong Dong (yddong.gm@gmail.com)
"""
...
class SimpleModelLODOCV(object):

    def __init__(self, trainArray, probeArray, directory, classifier=None):
        """
...
def calAUC(i):
    ...
    simpleModel = SimpleModelLODOCV(arrays[i][0], arrays[i][1], directory + "featuresLODOCV")
    simpleModel.readFeatures(i + 1)

    X_train, X_test, y_train, y_test = simpleModel.toNumArray(i)
    ...
    rocData = rocData.T
    np.savetxt(directory + "resultsLODOCV/" + method + "/rocData" + str(i + 1) + ".csv", rocData, delimiter=',')
    ...
    auprData = auprData.T
    np.savetxt(directory + "resultsLODOCV/" + method + "/auprData" + str(i + 1) + ".csv", auprData, delimiter=',')
...
def calFeatures(trainArray, directory, numFold, numFeatures):
    """
    ...
    asymModel = AsymModel(trainArray, directory, numFold, numFeatures)
...
if __name__ == '__main__':
    ...
    arrays = interactions.splitTrainProbeArrayBasedOnCol()
    featuresPath = directory + "featuresLODOCV/"

    print len(os.listdir(featuresPath))
    if len(os.listdir(featuresPath)) != numFold:
        print "calculate features"
        for i in range(numFold):
            calFeatures(arrays[i][0], directory, i + 1, numFeatures)
        processCalList = []
        for i in range(numFold):
            process = Process(target=calFeatures, args=(arrays[i][0], arrays[i][1], i + 1))
    ...
    for i in range(0, numFold):
        auc_S, aupr_S = calAUC(i)
        AUCList.append(auc_S)
        AUPRList.append(aupr_S)

    print sum(AUCList) * 1.0 / len(AUCList)
    print sum(AUPRList) * 1.0 / len(AUPRList)

    np.savetxt(directory + "resultsLODOCV/" + method + "/aucList.txt", AUCList)
    np.savetxt(directory + "resultsLODOCV/" + method + "/auprList.txt", AUPRList)
